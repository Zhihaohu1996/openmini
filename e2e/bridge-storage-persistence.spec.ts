...
test('openmini.storage.* persists across a real destroy -> reload cycle', async ({ page }) => {
  ...
  await frame.locator('body').evaluate(() =>
    (
      window as unknown as {
        __bridgeDemo: { setPersistedValue(key: string, value: string): Promise<void> };
      }
  ...
  await expect(frameAfterReload.locator('#storage-result')).toHaveText('hello from bridge', {
    timeout: 10_000,
  });
  ...
  const persistedValue = await frameAfterReload.locator('body').evaluate(() =>
    (
      window as unknown as {
        __bridgeDemo: { getPersistedValue(key: string): Promise<string | null> };
      }
    ).__bridgeDemo.getPersistedValue('persist-key'),
  );
