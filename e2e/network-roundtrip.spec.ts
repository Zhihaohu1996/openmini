...
test('an allowlisted cross-origin request round-trips through the host bridge', async ({
  page,
}) => {
  ...
  const attempt = await frame
    ...
    .evaluate(() =>
      (window as unknown as NetworkHooksWindow).__bridgeDemo.networkFetch(
        'http://localhost:5174/test-api/echo',
      ),
    );
...
test('a Mini App without the network permission cannot call network.fetch at all', async ({
  page,
}) => {
  ...
  const attempt = await frame
    ...
    .evaluate(() =>
      (window as unknown as NetworkHooksWindow).__bridgeDemo.networkFetch(
        'http://localhost:5173/test-api/echo',
      ),
    );
...
test('the sandbox CSP still blocks the Mini App from fetching directly', async ({ page }) => {
  ...
  const direct = await frame
    .locator('body')
    .evaluate(() =>
      (window as unknown as NetworkHooksWindow).__bridgeDemo.directFetch(
        'http://localhost:5173/test-api/echo',
      ),
    );
...
test('the host aborts a response over its own size cap', async ({ page }) => {
  ...
  const oversized = await frame
    .locator('body')
    .evaluate(() =>
      (window as unknown as NetworkHooksWindow).__bridgeDemo.networkFetch(
        'http://localhost:5173/test-api/large',
      ),
    );
