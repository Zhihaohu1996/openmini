...
test('destroying the sandbox mid-request causes no crash or unhandled rejection', async ({
  page,
}) => {
  ...
  await frame.locator('body').evaluate(() => {
    (
      window as unknown as { __bridgeDemo: { sendAfterClose(): void } }
    ).__bridgeDemo.sendAfterClose();
  });
